...
import {Button, Flex, Modal, PullToRefresh, Tabs, Toast, WhiteSpace,InputItem} from 'antd-mobile';
...
export default class Order extends React.Component {
    constructor(props, context) {
        ...
        this.state = {
            ...
            tab: 0,
            wechatId: localStorage.getItem("businessId"),
    ...
    requestAllOrder(wechatId, page, rows) {
        console.log("请求所有订单");
        //所有订单
        myApi.getmyAllOrderListByAccount(wechatId, page, rows, (rs) => {
    ...
    requestPayOrder(wechatId, page, rows) {
        console.log("请求待付款订单");
        //待付款订单
        myApi.getmyOrderListByAccount(wechatId, 0, page, rows, (rs) => {
    ...
    requestDeliverOrder(wechatId, page, rows) {
        ...
        //待发货订单
        myApi.getmyOrderListByAccount(wechatId, 1, page, rows, (rs) => {
            let order1 = rs.obj.rows;

            myApi.getmyOrderListByAccount(wechatId, 2, page, rows, (rs) => {
                let order2 = rs.obj.rows;
                order1 = order1.concat(order2);

                myApi.getmyOrderListByAccount(wechatId, 3, page, rows, (rs) => {
    ...
    requestReceiveOrder(wechatId, page, rows) {
        ...
        //待收货订单
        myApi.getmyOrderListByAccount(wechatId, 4, page, rows, (rs) => {
    ...
    requestEvaluateOrder(wechatId, page, rows) {
        ...
        //待评价订单
        let order = [];
        myApi.getmyOrderListByAccount(wechatId, 5, page, rows, (rs) => {
            let order1 = rs.obj.rows;
            let valid1 = [];
            ...
            myApi.getmyOrderListByAccount(wechatId, 6, page, rows, (rs) => {
    ...
    requestRefundOrder(wechatId, page, rows) {
        ...
        //退款订单
        myApi.getmyOrderListByAccount(wechatId, 8, page, rows, (rs) => {
            let order1 = rs.obj.rows;

            myApi.getmyOrderListByAccount(wechatId, 9, page, rows, (rs) => {
                let order2 = rs.obj.rows;
                order1 = order1.concat(order2);

                myApi.getmyOrderListByAccount(wechatId, 10, page, rows, (rs) => {
                    let order3 = rs.obj.rows;
                    order1 = order1.concat(order3);

                    myApi.getmyOrderListByAccount(wechatId, 11, page, rows, (rs) => {
                        let order4 = rs.obj.rows;
                        order1 = order1.concat(order4);

                        myApi.getmyOrderListByAccount(wechatId, 12, page, rows, (rs) => {
    ...
    delivery(orderId, deliveryName, deliveryCode){
        myApi.getmydelivery(orderId, deliveryName, deliveryCode, (rs) => {
            if (rs && rs.success) {
                Toast.info("发货成功！", 1);
                // history.go(0)
                this.requestDeliverOrder(this.state.wechatId, 1, 10);
            }
            else {
                Toast.info("发货失败！", 1);
            }
            ...
            case 2:
                if (item.orderState === 2)
                    ...
                    return <Button type="ghost" inline size="small" 
                                   ...
                                   <InputItem
                                   placeholder="请输入物流公司"
                                   onChange={v => this.setState({ wuliuCompany: v })}
                                   >
                                   物流公司
                                   </InputItem>
                                   <InputItem
                                   placeholder="请输入物流单号"
                                   onChange={v => this.setState({ wuliuCorde: v })}
                                   >
                                   物流单号
                                   </InputItem>
                                   </div> 
                                   ...
                                   [
                                       { text: '取消', onPress: () => {} },
                                       { text: '确认', onPress: () => {this.delivery(item.id,this.state.wuliuCompany,this.state.wuliuCorde)} },
                                   ])}>
                        发货
                    </Button>
                }
    ...
    getOrderContent(order, orderStateStr) {
        ...
        if (this.state.isNull)
            return <div className="tip">
                {
                    (OrderManager.checkOrder(this.state.tab)=="待付款" || OrderManager.checkOrder(this.state.tab)=="待收货")
                    ?<div>您还没有{OrderManager.checkOrder(this.state.tab)}客户订单哦！</div>
                    :OrderManager.checkOrder(this.state.tab)=="待评价"
                    ?<div>您还没有已完成订单哦！</div>
                            ...
                            <Link to={{
                                ...
                                <Flex style={{background: '#F7F7F7'}}>
                                    <Flex.Item style={{flex: '0 0 25%'}}>
                                        <img src={"http://" + getServerIp() + product.iconURL.mediumPath}
    ...
    render() {
        ...
        return <Layout header={false} footer={false}>
            ...
            <Navigation title="订单管理" left={true} backLink='/my'/>
